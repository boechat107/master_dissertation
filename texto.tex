...
medido pela acurácia ($E_a$) que é normalmente apresentada como o erro médio quadrático
(MSE) entre as predições e as medições dos sensores.
...
predição ($E_d$) para sensores conhecidamente falhos, com presença de desvios em suas medições.
Esta métrica é calculada pelo MSE entre a predição gerada pelo modelo regressivo, cujas
entradas são as medidas de sensores com desvio, e o valor verdadeiro da variável
monitorada pelo respectivo sensor, como apresentado na Eq. (\ref{eq:erro_predicao}).
Sistemas de predição com menores valores de $E_a$ não apresentam necessariamente menores
valores de $E_d$.

\begin{equation}
    E_d = \frac{1}{N} \sum\limits^N_{i=1} \left( r^{(i)}_{drift} - r^{(i)} \right)
