...
de dados de treinamento $\mathbf{X} \in \mathbb{R}^{n \times p}$, onde $n_m$ é número de amostras
...
\simbolo{$\mathbf{r}_{1 \times p}$}{Vetor que representa uma amostra de dados de
$p$ sensores}

\simbolo{$\hat{\mathbf{r}}_{1\times p}$}{Estimativa de $\mathbf{r}$ gerada por um modelo
regressivo}

\simbolo{$\mathbf{d}_{1 \times p}$}{Vetor de resíduos entre $\mathbf{r}$} e
$\hat{\mathbf{r}}$

\simbolo{$n_m$}{Quantidade de amostras de dados usados para construção dos modelos
regressivos}

\simbolo{$\mathbf{X}_{n_m \times p}$}{Matriz de dimensão $n_m \times p$ de histórico de
...
Considerando a arquitetura OSA-CBM, um sistema de validação de sensores básico poderia ser
divido em duas das seis camadas, a camada de processamento de sinal e a de monitoramento
da condição, como ilustrado na Fig.~\ref{fig:osa_drift}.
...
A camada de processamento de sinal seria responsável por extrair informações sobre a
condição de funcionamento dos sensores a partir das medições realizadas pelos mesmos e
fornecidas pela camada de aquisição de dados.
A comunicação com a base de dados seria essencial, tanto para o resgate do histórico de
medições dos sensores quanto para o armazenamento das informações extraídas das novas
amostras.
% Esta tarefa pode ser subdividida em três partes: o pré-processamento, o processamento
% principal e o pós-processamento.
As tarefas desta camada podem ser divididas em três partes:

\begin{description}
    \item[Pré-processamento:] consiste na preparação e condicionamento dos dados aos
        algoritmos de regressão.

    \item[Processamento ou predição:] responsável pela construção de modelos regressivos,
    predição de sinais e geração de resíduos.
...
A camada de monitoramento da condição realizaria a tarefa de monitorar os indicadores de
falhas nos sensores, os resíduos.
Mudanças nas propriedades estatísticas dos resíduos seriam detectadas, gerando alertas ou
alarmes que indicariam a ocorrência de falhas em sensores específicos.

Como indicado na Fig.~\ref{fig:osa_drift}, ambas as camadas poderiam apresentar resultados e
indicadores diretamente ao usuário através de uma interface gráfica.
...
utilizada, dada pela Eq.~(\ref{eq:aakr_distance}), onde $u_i$ é a distancia Euclidiana
entre o vetor de memória $\mathbf{X}_i$ e a entrada $\mathbf{r}$.

\simbolo{$u_i$}{Distância Euclidiana entre o vetor de memória $\mathbf{X}_i$ e um vetor de
entrada $\mathbf{r}$.}

\begin{equation}
    {u_i}\left( {{{\mathbf{X}}_i},{\mathbf{r}}} \right) = \sqrt {{{\left(
...
\begin{equation}
    \hat{r}_j = \frac{\sum\limits_{i = 1}^{n_m} {\overline{w}_i}\cdot
        X_{i,j}  }{{\sum\limits_{i = 1}^{{n_m}} {{\overline{w}_i}} }}
...
\mathbb{R}^{p}$, de $p$ variáveis independentes, é usada para estimar uma
função $f(\mathbf{r})$ que correlaciona as $p$ variáveis de $\mathbf{r}$ com uma variável
dependente $y \in \mathbb{R}$ a partir de $n_m$ observações independentes e identicamente
distribuídas.
...
\begin{equation}
    ...
    \sum\limits_{i,j=1}^n (\alpha_i - \alpha_i^*) (\alpha_j - \alpha_j^*) \; \textsf{K}(r_i, r_j)
...
onde $\alpha, \alpha^*$ são multiplicadores de Lagrange e $\textsf{K}(\mathbf{r}, \mathbf{r}_i)$ é
...
\begin{equation}
    ...
    \textsf{K}(\mathbf{r}, \mathbf{r}_i) + b
...
\begin{equation}
    \textsf{K}(\mathbf{r}, \mathbf{r}_i) = \exp \left( - \frac{||\mathbf{r}-\mathbf{r}_i||^2}{2 \gamma^2} \right)
...
\begin{equation}
    ...
    \hat{r}^{(i)}_{k, drift} \right|}{\left| r_{k,drift}^{(i)} - r^{(i)}_{k,drift} \right|}
...
desvio $k$, a sensibilidade cruzada é calculada por
...
\section{Ensaios com Dados de Simulação}
...
Sistema 1.
Inserindo desvios artificiais no conjunto de dados de otimização, foram selecionados os
...
Para tal análise, um desvio artificial linearmente crescente terminando com uma magnitude
...
mais independete dos valores de PT$_g$ (\textcolor{red}{confirmar com os valores das
correlações entre as varipaveis}), uma vez que suas predições praticamente não foram
...
artificiais, confirmando seu maior valor de $S_C$, entretanto, as predições de PT$_g$
...
\begin{figure}[bt]
    ...
    simulação, com inserção de desvios artificias na variável PT$_g$. Linhas vermelhas são
...
\section{Ensaios com Dados Reais}
...
sensibilidade e acurácia dos sistemas, avaliados com o conjunto de dados de teste.
Novamente, um desvio artificial linearmente crescente terminando com uma magnitude
de $3\sigma$ (três vezes o desvio padrão dos dados da variável) foi introduzido
sequencialmente (em uma variável por vez) no conjunto de dados de teste.
...
As estimativas dos três sistemas na condição de ocorrência de desvios artificiais no
...
\begin{figure}[bt]
    ...
    \caption{Predições dos sistemas para o conjunto de dados reais de testes, com inserção
        de desvios artificias nas leituras do PDG. Linhas vermelhas são as predições,
